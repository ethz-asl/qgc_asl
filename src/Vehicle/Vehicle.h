...
signals:
    ...
    void MPPTDataChanged(int* Failure, bool* FailureChanged,float volt1, float amp1, uint16_t pwm1, uint8_t status1, float volt2, float amp2, uint16_t pwm2, uint8_t status2, float volt3, float amp3, uint16_t pwm3, uint8_t status3);
    //void MPPTDataChangedHL(uint8_t volt1, uint8_t volt2, uint8_t volt3);
    void BatMonDataChanged(int* Failure, bool* FailureChanged, uint8_t compid, uint16_t volt, int16_t current, uint8_t soc, float temp, uint16_t batStatus, uint32_t safetystatus, uint32_t operationstatus, uint16_t cellvolt1, uint16_t cellvolt2, uint16_t cellvolt3, uint16_t cellvolt4, uint16_t cellvolt5, uint16_t cellvolt6);
    ...
    void SensPowerBoardChanged(int* Failure, bool* FailureChanged,uint64_t timestamp, uint8_t status, uint8_t led_status, float system_volt, float servo_volt, float digital_volt, float mot_l_amp, float mot_r_amp, float analog_amp, float digital_amp, float ext_amp, float aux_amp);
    void thrustChanged(Vehicle* vehicle, double thrust); //Needed for energy widget, but could be avoided if we add a fact
    void energySystemFailureChanged();
...
private:
    ...
    int _batmonFailure;
    int _mpptFailure;
    int _powerboardFailure;
    bool _energySystemFailureChanged;
